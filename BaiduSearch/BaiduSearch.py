...
def GetTitleUrl(url,res_title,res_url,bShowRes):
  try:
    ...
    for i in soup.findAll("h3"):
      ...
      if bShowRes==1:
          loggerbaidu.log.info('Number-:'+str(num))
          loggerbaidu.log.info('title-:'+temp_title)
          loggerbaidu.log.info('url-:'+realurl)
...
if __name__ == "__main__":
  ...
  for i in range(start,int(searchnum)+start):
    if ShowRes_Mode==1:
        loggerbaidu.log.info("第%s页内容：".decode("utf8").encode("gbk") %(str(int(i))))
    '''
    wd：搜索关键字
    rn：每页显示10条结果
    pn：第几条开始
  ...
  if ShowRes_Mode == 2:
      for i in range(len(res_title)):
        loggerbaidu.log.info('Number-:'+str(i+1))
        loggerbaidu.log.info('title-:'+res_title[i])
        loggerbaidu.log.info('url-:'+res_url[i])
